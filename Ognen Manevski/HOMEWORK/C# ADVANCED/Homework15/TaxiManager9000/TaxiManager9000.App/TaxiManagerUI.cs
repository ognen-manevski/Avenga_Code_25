            ...
            while (true)
            {
                ...
                switch (mainMenuChoce)
                {
                    ...
                    case MenuChoice.DriverManager:
                        ...
                        int driverManagerChoice = _uiService.ChooseMenu(driverManagerMenu);
                        var availableDrivers = _driverService.GetFiltered(x => _driverService.IsAvailableDriver(x));
                        if (driverManagerChoice == 1)
                        {
                            var availableForAssigningDrivers = availableDrivers
                                .Where(x => x.Car == null)
                                .ToList();
                            var assigningDrvierChoice = _uiService
                                .ChooseEntitiesMenu<Driver>(availableForAssigningDrivers);
                            if (assigningDrvierChoice == -1) continue;

                            var availableCarsForAssigning = _carService
                                .GetFiltered(x => _carService.IsAvailableCar(x))
                                .ToList();
                            var assigningCarChoice = _uiService
                                .ChooseEntitiesMenu<Car>(availableCarsForAssigning);
                            if (assigningCarChoice == -1) continue;

                            _driverService.AssignDriver(
                        ...
                        else if (driverManagerChoice == 2)
                        {
                            var availableForUnassigningDrivers = availableDrivers
                                .Where(x => x.Car != null)
                                .ToList();
                            var unassigningDrvierChoice = _uiService
                                .ChooseEntitiesMenu<Driver>(availableForUnassigningDrivers);
                            if (unassigningDrvierChoice == -1) continue;

                            _driverService.Unassign(availableForUnassigningDrivers[unassigningDrvierChoice - 1]);
                        }
                        break;
                    case MenuChoice.ListAllCars:
                        ConsoleHelper.PrintInColor("===== List All Cars", ConsoleColor.Cyan);
                        _carService.GetAll().Print();
                        break;
                    case MenuChoice.LicensePlateStatus:
                        _carService.GetAll().PrintStatus();
                        break;
