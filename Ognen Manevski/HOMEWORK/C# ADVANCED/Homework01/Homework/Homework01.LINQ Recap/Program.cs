//data
// Data Sample
using Homework01.LINQ_Recap.Models;
...
string? task6 = products
    .Where(p => p.Description.Contains("display")) //simple solution
                                                   //more robust solution in class01-exercise01
    .Select(p => p.Description)
    .LastOrDefault();

//7.Select the title of the first Laptop that has a stock greater than 80.

string? task7 = products
...
//copilot had to help here:
Dictionary<ProductCategory, List<Product>> productsByCategory = products
    .GroupBy(p => p.Category) //group products by category
    .ToDictionary(g => g.Key, g => g.ToList()); //convert the grouped result to a dictionary
                                                //where the key is the category and
                                                //the value is the list of products in that category

//Bonus

//17.Create new class ProductDetails with 3 properties: Id, Title and Price and map the existing product data to
//a collection of ProductDetails objects.

IEnumerable<ProductDetails> ProductDataCollection = products
